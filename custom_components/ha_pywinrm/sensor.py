...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    data = WinRMSensorData(hass, command, host, password, username, command_timeout)

    add_entities(
        [WinRMSensor(hass, data, name, unit, value_template, json_attributes)], True
    )


class WinRMSensor(Entity):
...
class WinRMSensorData:
    """The class for handling the data retrieval."""

    def __init__(self, hass, command, host, password, username, command_timeout):
    ...
    def update(self):
        ...
        try:
            _LOGGER.debug("Running command: %s on host: %s" % (command, host))
